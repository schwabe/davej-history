...
#define wd_writew(val, addr)	(addr = val)
#define wd_writeb(val, addr)	(addr = val)
#define wd_readw(addr)			(addr)
#define wd_readb(addr)			(addr)
...
void wd_dumpregs(void)
{
	...
	printk("\tintr_mask  at 0x%lx: 0x%x\n", 
		...
		wd_readb(wd_dev.regs->pld_regs.intr_mask));
	printk("\tpld_status at 0x%lx: 0x%x\n", 
		(unsigned long)(&wd_dev.regs->pld_regs.status), 
		wd_readb(wd_dev.regs->pld_regs.status));
}
...
ebus_done:
	...
	if(check_region(edev->base_address[0], sizeof(*wd_dev.regs)))
	{
		printk("%s: Unable to get region %lx, %d\n", 
			__FUNCTION__, edev->base_address[0], (int)sizeof(*wd_dev.regs));
	}

	wd_dev.regs = 
		(struct wd_regblk *)edev->base_address[0];
	request_region(
		(unsigned long)wd_dev.regs, sizeof(*wd_dev.regs), WD_OBPNAME);
...
void cleanup_module(void)
{
	...
	release_region((unsigned long)wd_dev.regs, sizeof(*wd_dev.regs));
}
#endif /* ifdef MODULE */
