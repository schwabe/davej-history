...
static void do_cciss_request0(void) DO_CCISS_REQUEST(0);
static void do_cciss_request1(void) DO_CCISS_REQUEST(1);
static void do_cciss_request2(void) DO_CCISS_REQUEST(2);
static void do_cciss_request3(void) DO_CCISS_REQUEST(3);
static void do_cciss_request4(void) DO_CCISS_REQUEST(4);
static void do_cciss_request5(void) DO_CCISS_REQUEST(5);
static void do_cciss_request6(void) DO_CCISS_REQUEST(6);
static void do_cciss_request7(void) DO_CCISS_REQUEST(7);
...
static struct file_operations cciss_fops  = {
        NULL,                        /* lseek - default */
        ...
        cciss_ioctl,                  /* ioctl */
        NULL,                        /* mmap */
        cciss_open,                     /* open code */
        NULL,
        cciss_release,                  /* release */
        block_fsync,                  /* fsync */
        NULL,                        /* fasync */
        NULL,                   /* Disk change */
...
static void cciss_procinit(int i)
{
        ...
        if (proc_cciss == NULL) {
                proc_cciss = create_proc_entry("cciss", S_IFDIR|S_IRUGO|S_IXUGO,                                                                &proc_root);
                if (!proc_cciss) 
			return;
        }

        pd = create_proc_entry(hba[i]->devname, S_IFREG|S_IRUGO, proc_cciss);
        if (!pd) 
		return;
        pd->read_proc = cciss_proc_get_info;
        pd->data = hba[i];
}
 ...
 */
static void cciss_geninit( struct gendisk *g)
{
	int ctlr = -1; 
	drive_info_struct *drv;
	int i,j;

	/* find the controller this gendisk data is from */ 
	for(i=0; i < nr_ctlr; i++)
	{
		if ( g == &(hba[i]->gendisk))
		{
	...
	if ( ctlr < 0) 
	{
		printk(KERN_WARNING "cciss: gendisk data invalid\n");
		return;
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case CCISS_GETPCIINFO:
	{
		...
		copy_to_user_ret((void *) arg, &pciinfo,  
			sizeof( cciss_pci_info_struct),  -EFAULT);
		return(0);
	}	
	case CCISS_GETINTINFO:
	{
		...
		copy_to_user_ret((void *) arg, &intinfo,
                        sizeof( cciss_coalint_struct),  -EFAULT);
                return(0);
        }
	case CCISS_SETINTINFO:
		...
		copy_from_user_ret(&intinfo, (void *) arg, 
			sizeof( cciss_coalint_struct),  -EFAULT);
...
//					c->commands_outstanding);
	...
	case CCISS_GETNODENAME:
		...
		for(i=0;i<16;i++)
			NodeName[i] = readb(&c->cfgtable->ServerName[i]);
                copy_to_user_ret((void *) arg, NodeName,
                        sizeof( NodeName_type),  -EFAULT);
                return(0);
        }
	case CCISS_SETNODENAME:
	{
		...
		copy_from_user_ret(NodeName, (void *) arg, 
			sizeof( NodeName_type),  -EFAULT);
	...
	case CCISS_GETHEARTBEAT:
		...
		if (!arg) return -EINVAL;
                ...
                copy_to_user_ret((void *) arg, &heartbeat,
                        sizeof( Heartbeat_type),  -EFAULT);
                return(0);
        }
	case CCISS_GETBUSTYPES:
		...
		if (!arg) return -EINVAL;
                ...
                copy_to_user_ret((void *) arg, &BusTypes,
                        sizeof( BusTypes_type),  -EFAULT);
                return(0);
        }
	case CCISS_GETFIRMVER:
		...
		memcpy(firmware, hba[ctlr]->firm_ver, 4);
                ...
                copy_to_user_ret((void *) arg, firmware,
                        sizeof( FirmwareVer_type),  -EFAULT);
                return(0);
        }
		...
		DriverVer_type DriverVer = DRIVER_VERSION;
                ...
                copy_to_user_ret((void *) arg, &DriverVer,
                        sizeof( DriverVer_type),  -EFAULT);
                return(0);
        }
	...
	case CCISS_PASSTHRU:
	{
		...
		copy_from_user_ret(&iocommand, (void *) arg,
                        sizeof( IOCTL_Command_struct),  -EFAULT); 
		...
		if (iocommand.Request.Type.Direction == XFER_WRITE)
		{
			...
			copy_from_user_ret(buff, iocommand.buf,
                        	iocommand.buf_size,  -EFAULT);
		}
...
static int revalidate_allvol(kdev_t dev)
{
	...
	memset(hba[ctlr]->hd,         0, sizeof(struct hd_struct) * 256);
        ...
        cciss_geninit(&(hba[ctlr]->gendisk));
        for(i=0; i<NWD; i++)
...
static void do_cciss_request(int ctlr)
{
	...
	/* loop till creq is null or current_request is inactive */
...
int init_module(void)
{
	...
	for(i=0; i<nr_ctlr; i++)
	{
		cciss_geninit(&(hba[i]->gendisk));
		for(j=0; j<NWD; j++)
		{
			if(hba[i]->sizes[j<<NWD_SHIFT])
				resetup_one_dev(&(hba[i]->gendisk), j);
		}
	}
	return 0;
}
