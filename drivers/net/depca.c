...
__initfunc(static int depca_hw_init(struct device *dev, u_long ioaddr, int mca_slot))
{
	...
	if (inw(DEPCA_DATA) == STOP) {
		...
		if ((adapter != unknown) && mem_start) {	/* found a DEPCA device */
			...
			if (status == 0) {
				...
				if ((mem_len = (NUM_RX_DESC * (sizeof(struct depca_rx_desc) + RX_BUFF_SZ) + NUM_TX_DESC * (sizeof(struct depca_tx_desc) + TX_BUFF_SZ) + sizeof(struct depca_init))) <= (netRAM << 10)) {
...
*/
__initfunc(static void mca_probe(struct device *dev, u_long ioaddr))
{
	unsigned char pos[2];
	unsigned char where;
...
** the current list.
*/
__initfunc(static struct device * alloc_device(struct device *dev, u_long iobase))
{
...
*/
__initfunc(static struct device * insert_device(struct device *dev, u_long iobase, int (*init) (struct device *)))
{
	struct device *new;
...
__initfunc(static int depca_dev_index(char *s))
{
	int i = 0, j = 0;
