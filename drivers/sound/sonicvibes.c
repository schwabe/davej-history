	...
	if (!db->rawbuf) {
		db->ready = db->mapped = 0;
                ...
                for (order = DMABUF_DEFAULTORDER; order >= DMABUF_MINORDER && !db->rawbuf; order--)
                        db->rawbuf = (void *)__get_free_pages(GFP_KERNEL, order, 1);
#else
                for (order = DMABUF_DEFAULTORDER; order >= DMABUF_MINORDER && !db->rawbuf; order--)
...
#if LINUX_VERSION_CODE < 131328
static int sv_llseek(struct inode *ino, struct file *file, off_t offset, int origin)
{
        return -ESPIPE;
}
#else
...
#if LINUX_VERSION_CODE < 131328
static int sv_read(struct inode *ino, struct file *file, char *buffer, int count)
#else
...
#if LINUX_VERSION_CODE < 131328
static int sv_write(struct inode *ino, struct file *file, const char *buffer, int count)
#else
...
/* well select really */
static int sv_poll(struct inode *inode, struct file *file, int sel_type, select_table *wait)
{
        struct sv_state *s = (struct sv_state *)file->private_data;
        unsigned long flags;
        int ret = 0;
        ...
        if (sel_type == SEL_OUT && file->f_mode & FMODE_WRITE) {
                ...
                if (ret)
                        return 1;
                select_wait(&s->dma_dac.wait, wait);
        }
        return 0;
}
...
#if LINUX_VERSION_CODE < 131328
static int sv_mmap(struct inode *ino, struct file *file, struct vm_area_struct *vma)
#else
...
#if LINUX_VERSION_CODE < 131328
static int sv_midi_read(struct inode *ino, struct file *file, char *buffer, int count)
#else
...
#if LINUX_VERSION_CODE < 131328
static int sv_midi_write(struct inode *ino, struct file *file, const char *buffer, int count)
#else
...
/* well select really */
static int sv_midi_poll(struct inode *inode, struct file *file, int sel_type, select_table *wait)
{
        struct sv_state *s = (struct sv_state *)file->private_data;
        unsigned long flags;
        int ret = 0;
...
#endif
{
	...
	if (!(wavetable_mem = __get_free_pages(GFP_KERNEL, 20-PAGE_SHIFT)))
...
#endif
        for (index = 0; index < NR_DEVICE; index++) {
                ...
                if (vendid != PCI_VENDOR_ID_S3 || devid != PCI_DEVICE_ID_S3_SONICVIBES)
                        continue;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_1, &ioaddr);
                if (ioaddr == 0 || (ioaddr & PCI_BASE_ADDRESS_SPACE) != PCI_BASE_ADDRESS_SPACE_IO)
                        continue;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_2, &ioaddr);
                if (ioaddr == 0 || (ioaddr & PCI_BASE_ADDRESS_SPACE) != PCI_BASE_ADDRESS_SPACE_IO)
                        continue;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_2, &ioaddr);
	                if (ioaddr == 0 || (ioaddr & PCI_BASE_ADDRESS_SPACE) != PCI_BASE_ADDRESS_SPACE_IO)
		...
		s->open_sem = MUTEX;
		s->magic = SV_MAGIC;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_0, &s->iosb);
                s->iosb &= PCI_BASE_ADDRESS_IO_MASK;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_1, &s->ioenh);
                s->ioenh &= PCI_BASE_ADDRESS_IO_MASK;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_2, &s->iosynth);
                s->iosynth &= PCI_BASE_ADDRESS_IO_MASK;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_3, &s->iomidi);
                s->iomidi &= PCI_BASE_ADDRESS_IO_MASK;
                pcibios_read_config_dword(bus, dev_fn, PCI_BASE_ADDRESS_4, &s->iogame);
                s->iogame &= PCI_BASE_ADDRESS_IO_MASK;
                pcibios_read_config_dword(bus, dev_fn, 0x40, &s->iodmaa);
                pcibios_read_config_dword(bus, dev_fn, 0x48, &s->iodmac);
		...
		if (!(s->iodmac)) {
			...
			printk(KERN_INFO "sv: BIOS did not allocate DDMA channel C io, allocated at %#x\n",
                ...
                pcibios_write_config_dword(bus, dev_fn, 0x40, s->iodmaa | 9);  /* enable and use extended mode */
                pcibios_write_config_dword(bus, dev_fn, 0x48, s->iodmac | 9);  /* enable */
		...
		/* hack */
		pcibios_write_config_dword(bus, dev_fn, 0x60, wavetable_mem >> 12);  /* wavetable base address */
