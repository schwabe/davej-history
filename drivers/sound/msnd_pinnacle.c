...
static int probe_multisound(void)
{
	char *xv, *rev = NULL;
...
static int calibrate_adc(WORD srate)
{
...
static int attach_multisound(void)
{
	int err;
...
static int irq =			-1;
static int mem =			-1;
static int fifosize =			DEFFIFOSIZE;
static int calibrate_signal;

int init_module(void)
#else
static int io =				CONFIG_MSNDPIN_IO;
static int irq =			CONFIG_MSNDPIN_IRQ;
static int mem =			CONFIG_MSNDPIN_MEM;
static int fifosize =			DEFFIFOSIZE;
static int calibrate_signal;

int msnd_pinnacle_init(void)
#endif
{
	int err;
