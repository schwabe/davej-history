...
static int sound_write(struct inode *inode, struct file *file, const char *buf, int count)
{
	int dev = MINOR(inode->i_rdev);
...
static int sound_select(struct inode *inode, struct file *file, int sel_type, 
	select_table * wait)
{
	int dev = MINOR(inode->i_rdev);
	...
	case SND_DEV_SEQ:
	case SND_DEV_SEQ2:
		return sequencer_select(dev, file, sel_type, wait);
#endif

#ifdef CONFIG_MIDI
	case SND_DEV_MIDIN:
		return MIDIbuf_select(dev, file, sel_type, wait);
#endif
	...
	case SND_DEV_DSP16:
	case SND_DEV_AUDIO:
		return DMAbuf_select(file, dev >> 4, sel_type, wait);
#endif
	}
	return 0;
}

static int sound_mmap(struct inode *inode, struct file *file, struct vm_area_struct *vma)
{
	int dev_class;
	unsigned long size;
