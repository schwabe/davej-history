...
#if LINUX_VERSION_CODE < 131328
static int es1371_llseek(struct inode *ino, struct file *file, off_t offset, int origin)
{
        return -ESPIPE;
}
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1371_read(struct inode *ino, struct file *file, char *buffer, int count)
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1371_write(struct inode *ino, struct file *file, const char *buffer, int count)
#else
...
/* well select really */
static int es1371_poll(struct inode *inode, struct file *file, int sel_type, select_table *wait)
{
        struct es1371_state *s = (struct es1371_state *)file->private_data;
        unsigned long flags;
        int ret = 0;
...
#if LINUX_VERSION_CODE < 131328
static int es1371_mmap(struct inode *ino, struct file *file, struct vm_area_struct *vma)
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1371_write_dac(struct inode *ino, struct file *file, const char *buffer, int count)
#else
...
/* well select really */
static int es1371_poll_dac(struct inode *inode, struct file *file, int sel_type, select_table *wait)
{
        struct es1371_state *s = (struct es1371_state *)file->private_data;
        unsigned long flags;
        int ret = 0;
...
#if LINUX_VERSION_CODE < 131328
static int es1371_mmap_dac(struct inode *ino, struct file *file, struct vm_area_struct *vma)
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1371_midi_read(struct inode *ino, struct file *file, char *buffer, int count)
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1371_midi_write(struct inode *ino, struct file *file, const char *buffer, int count)
#else
...
/* well select really */
static int es1371_midi_poll(struct inode *inode, struct file *file, int sel_type, select_table *wait)
{
        struct es1371_state *s = (struct es1371_state *)file->private_data;
        unsigned long flags;
        int ret = 0;
...
#endif
{
	...
	printk(KERN_INFO "es1371: version v0.2 time " __TIME__ " " __DATE__ "\n");
        for (index = 0; index < NR_DEVICE; index++) {
                ...
                if (vendid != PCI_VENDOR_ID_ENSONIQ || devid != PCI_DEVICE_ID_ENSONIQ_ES1371)
                        continue;
