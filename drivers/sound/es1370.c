 ...
 *    06.07.98   0.11  Reversed the mixer changes by Gerald Britton, as they
 *                     are bogus. According to OSS API docs, MIXER_WRITE ioctls
 *                     return the actual audio level of the hardware, i.e. rounded
 *                     to the next possible hardware volume step. Gerald's patch
 *                     effectively just stored and returned the requested value by
 *                     the application.
 *
...
#if LINUX_VERSION_CODE < 131328
static int es1370_mmap_dac(struct inode *ino, struct file *file, struct vm_area_struct *vma)
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1370_midi_read(struct inode *ino, struct file *file, char *buffer, int count)
#else
...
#if LINUX_VERSION_CODE < 131328
static int es1370_midi_write(struct inode *ino, struct file *file, const char *buffer, int count)
#else
...
/* well select really */
static int es1370_midi_poll(struct inode *inode, struct file *file, int sel_type, select_table *wait)
{
        struct es1370_state *s = (struct es1370_state *)file->private_data;
        unsigned long flags;
        int ret = 0;
...
#endif
{
	...
	if (!pcibios_present())   /* No PCI bus in this machine! */
		return -ENODEV;
	printk(KERN_INFO "es1370: version v0.11 time " __TIME__ " " __DATE__ "\n");
