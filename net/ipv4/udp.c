 ...
 *		Alan Cox	:	udp_send verify_area moved to avoid mem
 *					leak
 *		Alan Cox	:	UDP can count its memory
 *		Alan Cox	:	send to an unknown connection causes
 *					an ECONNREFUSED off the icmp, but
 *					does NOT close.
 *		Alan Cox	:	Switched to new sk_buff handlers. No
 *					more backlog!
 *		Alan Cox	:	Using generic datagram code. Even
 *					smaller and the PEEK bug no longer
 *					crashes it.
 *		Fred Van Kempen	:	Net2e support for sk->broadcast.
 *		Alan Cox	:	Uses skb_free_datagram
 *		Alan Cox	:	Added get/set sockopt support.
 *		Alan Cox	:	Broadcasting without option set returns
 *					EACCES.
 *		Alan Cox	:	No wakeup calls. Instead we now use the
 *					callbacks.
 ...
 *		Alan Cox	:	Incorrect return on truncated datagram
 *					receive.
...
/* RFC1122 Status:
   ...
   4.1.3.5 (UDP Multihoming)
     ...
     SHOULD be able to communicate the chosen src addr up to application
       when application doesn't choose (NOT YET - doesn't seem to be in the BSD
       API)
   ...
   4.1.3.6 (Invalid Addresses)
     ...
     MUST only send datagrams with one of our addresses (NOT YET - ought to be
     OK)
   950728 -- MS
*/
...
__inline__ struct sock *udp_v4_lookup(u32 saddr, u16 sport, u32 daddr, u16 dport, struct device *dev)
{
	...
	for (sk = udp_hash[hnum & (UDP_HTABLE_SIZE - 1)];
	     sk != NULL; sk = sk->next) {
		if ((sk->num == hnum) && !(sk->dead &&
		    (sk->state == TCP_CLOSE))) {
			int score = 0;
			if (sk->rcv_saddr) {
...
/*
 *	Copy and checksum a UDP packet from user space into a buffer. We still
 *	have to do the planning to get ip_build_xmit to spot direct transfer to
 *	network card and provide an additional callback mode for direct
 *	user->board I/O transfers. That one will be fun.
 */
...
/*
 *	Unchecksummed UDP is sufficiently critical to stuff like ATM video
 *	conferencing that we use two routines for this for speed. Probably we
 *	ought to have a CONFIG_FAST_NET set for >10Mb/second boards to activate
 *	this sort of coding. Timing needed to verify if this is a valid
 *	decision.
 */
		    ...
		    __u32 saddr, int noblock)
{
	...
	if (sk->no_check)
		a = ip_build_xmit(sk, udp_getfrag_nosum, &ufh, ulen,
				  sin->sin_addr.s_addr, saddr, sk->opt, rt,
				  IPPROTO_UDP, noblock);
	else
	   ...
	   unsigned flags, struct sockaddr_in *usin, int addr_len)
{
	...
	if (!sk->broadcast &&
	    ip_chk_addr(usin->sin_addr.s_addr) == IS_BROADCAST)
...
static int udp_sendmsg(struct sock *sk, struct msghdr *msg, int len, int noblock, int flags)
{
	if (msg->msg_iovlen == 1)
	     ...
	     int noblock, int flags, int *addr_len)
{
	...
	skb_copy_datagram_iovec(skb, sizeof (struct udphdr),
				msg->msg_iov, copied);
	sk->stamp = skb->stamp;
...
int udp_connect(struct sock *sk, struct sockaddr_in *usin, int addr_len)
{
	...
	rt = ip_rt_route((__u32)usin->sin_addr.s_addr,
			 sk->localroute, sk->bound_device);
	if (rt == NULL)
		return -ENETUNREACH;
	if (!sk->saddr)
				 ...
				 u32 saddr, u32 daddr)
{
	...
	sk = udp_v4_mcast_next(sk, uh->dest, saddr, uh->source,
			       daddr, skb->dev);
	if (sk) {
	...
	__u32 saddr, int redo, struct inet_protocol *protocol)
{
	...
	/* Skip csum_partial if CHECKSUM_UNNECESSARY */
	if (uh->check &&
