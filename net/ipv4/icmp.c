...
static void icmp_glue_bits(const void *p, __u32 saddr, char *to, unsigned int offset, unsigned int fraglen)
{
	...
	if (offset) {
		icmp_param->csum = csum_partial_copy(icmp_param->data_ptr +
						     offset -
...
void icmp_send(struct sk_buff *skb_in, int type, int code, unsigned long info, struct device *dev, int hdrincl)
{
	struct iphdr *iph;
	struct icmphdr *icmph;
	int atype, room;
	...
	if (icmp_pointers[type].error) {
		/*
		 *	We are an error,
		 *	check if we are replying to an ICMP error
		 */
		...
		if (iph->protocol == IPPROTO_ICMP) {
			 ...
			 *	This isn't specified by the RFC,
			 *	but think about it..
			 */
			if (icmph->type > 18 ||
			    icmp_pointers[icmph->type].error)
				return;
		}
	}
	...
	if (saddr != dev->pa_addr && ip_chk_addr(saddr) != IS_MYADDR)
		saddr = dev->pa_addr;
	if (ip_options_echo(&icmp_param.replyopts, NULL, saddr,
			    iph->saddr, skb_in))
		return;
	...
	icmp_param.data_len = skb_in->len;
	if (!hdrincl)
		icmp_param.data_len += (iph->ihl << 2);
...
static void icmp_unreach(struct icmphdr *icmph, struct sk_buff *skb, struct device *dev, __u32 saddr, __u32 daddr, int len)
{
	...
	if (icmph->type == ICMP_DEST_UNREACH) {
		...
		case ICMP_FRAG_NEEDED:
			...
			NETDEBUG(printk(KERN_INFO
					"ICMP: %s: fragmentation needed and DF set.\n",
					in_ntoa(iph->daddr)));
			break;
#else
		{
			 ...
			 * is not less than current mtu, reduce old_mtu by
			 * 4 times the header length.
			 */
	...
	if (!match_addr || saddr == iph->daddr) {
		...
		while (ipprot != NULL) {
			...
			if (iph->protocol == ipprot->protocol &&
			    ipprot->err_handler) {
				ipprot->err_handler(icmph->type, icmph->code,
						    dp, iph->daddr, iph->saddr,
						    ipprot, len);
			}
...
static void icmp_redirect(struct icmphdr *icmph, struct sk_buff *skb, struct device *dev, __u32 source, __u32 daddr, int len)
{
	...
	/*
	 *	If we are a router and we run a routing protocol, we MUST NOT
	 *	follow redirects.
	...
	if (sysctl_ip_forward) {
		NETDEBUG(printk(KERN_INFO
				"icmp: ICMP redirect ignored. dest = %lX, "
				"orig gw = %lX, \"new\" gw = %lX, device = %s.\n",
				ntohl(ip), ntohl(source),
				ntohl(icmph->un.gateway), dev->name));
		goto flush_it;
	}
#endif
	switch (icmph->code & 7) {
	case ICMP_REDIR_NET:
		/*
		 *	This causes a problem with subnetted networks. What we
		 *	should do is use ICMP_ADDRESS to get the subnet mask of
		 *	the problem route and set both. But we don't.. [RFC1812
		 *	says routers MUST NOT generate Network Redirects]
		 */
	...
	case ICMP_REDIR_HOST:
		/*
		 *	Add better route to host.  But first check that the
		 *	redirect comes from the old gateway..  And make sure
		 *	it's an ok host address (not some confused thing
		 *	sending our address)
		 */
		NETDEBUG(printk(KERN_INFO
				"ICMP redirect from %s\n", in_ntoa(source)));
		ip_rt_redirect(source, ip, icmph->un.gateway, dev);
		break;
	case ICMP_REDIR_NETTOS:
	case ICMP_REDIR_HOSTTOS:
		NETDEBUG(printk(KERN_INFO
				"ICMP: cannot handle TOS redirects yet!\n"));
		break;
	default:
		break;
	}
...
static void icmp_echo(struct icmphdr *icmph, struct sk_buff *skb, struct device *dev, __u32 saddr, __u32 daddr, int len)
{
	...
	if (ip_options_echo(&icmp_param.replyopts,
			    NULL, daddr, saddr, skb) == 0)
...
int icmp_chkaddr(struct sk_buff *skb)
{
	...
	void (*handler)(struct icmphdr *icmph, struct sk_buff *skb,
			struct device *dev, __u32 saddr, __u32 daddr,
			int len) = icmp_pointers[icmph->type].handler;
	...
	if (handler == icmp_unreach || handler == icmp_redirect) {
		...
		case IPPROTO_UDP:
		{
			...
			sk = udp_v4_lookup(iph->daddr, uh->dest, iph->saddr,
					   uh->source, skb->dev);
			if (!sk)
				return 0;
			if (sk->saddr != iph->saddr &&
			    ip_chk_addr(iph->saddr) != IS_MYADDR)
				return 0;
			/*
	 ...
	 __u32 saddr, int redo, struct inet_protocol *protocol)
{
	...
	if (daddr != dev->pa_addr && ip_chk_addr(daddr) != IS_MYADDR) {
		 ...
		 *	silently ignored (we don't as it is used by some
	...
	(icmp_pointers[icmph->type].handler)(icmph, skb, skb->dev, saddr,
					     daddr, len);
	return 0;
}

/*
 *	This table defined limits of ICMP sending rate for various ICMP
 *	messages.
 */
